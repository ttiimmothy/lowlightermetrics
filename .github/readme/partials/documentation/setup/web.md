...
```shell
wget https://raw.githubusercontent.com/actionhooks/lowlightermetrics/master/settings.example.json
...
Docker images are published on [GitHub Container Registry](https://github.com/actionhooks/lowlightermetrics/pkgs/container/metrics).
...
```shell
docker run --rm --entrypoint="" -p=127.0.0.1:$PUBLISHED_PORT:$SERVICE_PORT --volume=$SETTINGS:/metrics/settings.json ghcr.io/actionhooks/lowlightermetrics:$VERSION npm start
```
